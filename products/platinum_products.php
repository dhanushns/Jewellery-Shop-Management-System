<div class="title">
  <label>Platinum Products</label>
</div>
<div class="container1" id='chains'>
  <hr>
  <div class="container-title">Platinum Chains</div>
  <div class="products chain">
    <?php
      set_error_handler('plt_session_notice_handler');

      function plt_session_notice_handler($severity, $message, $filename, $lineno) {
      ...
      $sql_cmd = "SELECT COUNT(category) from products WHERE category = 'Platinum Chain'";
      $result = $conn->query($sql_cmd);
      $row = mysqli_fetch_array($result);
      $count = $row[0];
      $sql_cmd = "SELECT * FROM products WHERE category = 'Platinum Chain'";
      $result = $conn->query("$sql_cmd");
      $session_login = 0;
        ...
        for ($i = 1; $i <= $count; $i++) {
          ...
          echo "<div class='group'>
              ...
              <button style='border:none;background-color:transparent;' data-bs-toggle='modal' data-bs-target='#modal_$id'>
                <img src='Platinum/Chain/PC00$i.jpeg'>
              </button>
...
<div class="container1" id='earrings'>
  <div class="container-title">Platinum Earrings</div>
  <div class="products earrings">
    <?php
      $sql_cmd = "SELECT COUNT(category) from products WHERE category = 'Platinum Earring'";
      $result = $conn->query($sql_cmd);
      $row = mysqli_fetch_array($result);
      $count = $row[0];
      $sql_cmd = "SELECT * FROM products WHERE category = 'Platinum Earring'";
      $result = $conn->query("$sql_cmd");
      $session_login = 0;
     ...
     for ($i = 1; $i <= $count; $i++) {
      ...
      echo "<div class='group'>
          ...
          <button style='border:none;background-color:transparent;' data-bs-toggle='modal' data-bs-target='#modal_$id'>
            <img src='Platinum/Earring/PE00$i.jpeg'>
          </button>
          <div class='text-wrapper price'>$name  </div>
          <div class='text-wrapper price'>&#8377 $price</div>
        </div>";
...
<div class="container1" id='rings'>
  <div class="container-title">Platinum Rings</div>
  <div class="products rings">
    <?php
      $sql_cmd = "SELECT COUNT(category) from products WHERE category = 'Platinum Ring'";
      $result = $conn->query($sql_cmd);
      $row = mysqli_fetch_array($result);
      $count = $row[0];
      $sql_cmd = "SELECT * FROM products WHERE category = 'Platinum Ring'";
      $result = $conn->query("$sql_cmd");
      $session_login = 0;
      ...
      if(isset($_SESSION['username'])){
       ...
       for ($i = 1; $i <= $count; $i++) {
        ...
        echo "<div class='group'>
            ...
            <button style='border:none;background-color:transparent;' data-bs-toggle='modal' data-bs-target='#modal_$id'>
              <img src='Platinum/Ring/PR00$i.jpeg'>
            </button>
            <div class='text-wrapper price'>$name  </div>
            <div class='text-wrapper price'>&#8377 $price</div>
          </div>";
