<div class="title">
  <label>Silver Products</label>
</div>
<div class="container1" id="chains">
  <hr>
  <div class="container-title">Silver Chains</div>
  <div class="products chain">
    <?php
    set_error_handler('silver_session_notice_handler');

    function silver_session_notice_handler($severity, $message, $filename, $lineno) {
    ...
    try {
      ...
      $sql_cmd = "SELECT COUNT(category) from products WHERE category = 'Silver Chain'";
      $result = $conn->query($sql_cmd);
      $row = mysqli_fetch_array($result);
      $count = $row[0];
      $sql_cmd = "SELECT * FROM products WHERE category = 'Silver Chain'";
      $result = $conn->query("$sql_cmd");
      $session_login = 0;
        ...
        for ($i = 1; $i <= $count; $i++) {
          ...
          echo "<div class='group'>
              ...
              <button style='border:none;background-color:transparent;' data-bs-toggle='modal' data-bs-target='#modal_$id'>
                <img src='Silver/Chain/SC00$i.jpeg'>
              </button>
...
<div class="container1" id='bracelets'>
  <div class="container-title">Silver Bracelets</div>
  <div class="products bracelets">
    <?php
      $sql_cmd = "SELECT COUNT(category) from products WHERE category = 'Silver Bracelet'";
      $result = $conn->query($sql_cmd);
      $row = mysqli_fetch_array($result);
      $count = $row[0];
      $sql_cmd = "SELECT * FROM products WHERE category = 'Silver Bracelet'";
      $result = $conn->query("$sql_cmd");
      $session_login = 0;
     ...
     for ($i = 1; $i <= $count; $i++) {
      ...
      echo "<div class='group'>
          ...
          <button style='border:none;background-color:transparent;' data-bs-toggle='modal' data-bs-target='#modal_$id'>
            <img src='Silver/Bracelet/SB00$i.jpeg'>
          </button>
          <div class='text-wrapper price'>$name  </div>
          <div class='text-wrapper price'>&#8377 $price</div>
        </div>";
...
<div class="container1" id='rings'>
  <div class="container-title">Silver Rings</div>
  <div class="products rings">
    <?php
      $sql_cmd = "SELECT COUNT(category) from products WHERE category = 'Silver Ring'";
      $result = $conn->query($sql_cmd);
      $row = mysqli_fetch_array($result);
      $count = $row[0];
      $sql_cmd = "SELECT * FROM products WHERE category = 'Silver Ring'";
      $result = $conn->query("$sql_cmd");
      $session_login = 0;
      ...
      if(isset($_SESSION['username'])){
       ...
       for ($i = 1; $i <= $count; $i++) {
        ...
        echo "<div class='group'>
            ...
            <button style='border:none;background-color:transparent;' data-bs-toggle='modal' data-bs-target='#modal_$id'>
              <img src='Silver/Ring/SR00$i.jpeg'>
            </button>
            <div class='text-wrapper price'>$name  </div>
            <div class='text-wrapper price'>&#8377 $price</div>
          </div>";
